...
class admission_cancel
{
    function insert($enroll, $name, $fathername, $course, $sem, $mobile, $email)
    {
        $conn = connect();
        $sql = "INSERT INTO `admission_cancel`(`enrollment_no`, `name`, `father_name`, `course`,  `semester` ,  `mobile_no` ,  `email` ) VALUES ('$enroll','$name','$fathername','$course','$sem','$mobile','$email')";

        $bona = mysqli_query($conn, $sql);
        if ($bona) {
            # code...
            header("Location: private/forms/admission_cancel_form.pdf");
        } else {
 ...
 if (isset($_POST['course_code'])) {
     ...
     $result = $admission_cancel->fetch_semesters_from_table($course_code);
     for ($i = 1; $i <= $result; $i++) {
         if ($i == "1") {
