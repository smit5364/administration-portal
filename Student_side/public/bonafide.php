...
<body style="font-family: 'Geologica', sans-serif;">
    <div class="shadow-lg shadow-gray-100">
        <nav class="bg-white py-2 flex justify-center">
            <ul>
                <li><img src="private/images/logo.png" alt="bmu" class="w-32"></li>
            </ul>
        </nav>
    </div>
                ...
                <div class="grid grid-cols-4 mt-8 gap-x-10">
                    ...
                    <div class="row-span-2 flex flex-col gap-y-1 ">
                        ...
                        <select required name="semester" id=""
                            ...
                            <?php
                            if (!empty($crs)) {
                                $semesters = $bonafide->fetch_semesters_from_table($crs);
                                for ($i = 1; $i <= $semesters; $i++) {
                                    ...
                                    if ($i >= 1 && $i <= 8) {
                                        $year = ceil($i / 2);
                                        $dissem = $semesterInfo[$i];
                                    }

                                    echo '<option value="' . $i . '"' . $selected . '>' . $dissem . '</option>';
                                }
                            }
                            ?>
                        </select>
